...
class TradingApi {
  ...
  Future<Map<String, dynamic>> momentumPortfolio(String accountId) async {
    final data = await _client.get('/trading/accounts/$accountId/momentum/portfolio');
    return (data as Map).cast<String, dynamic>();
  }
  ...
  Future<List<Candidate>> scan({int limit = 20}) async {
    final data = await _client.get('/trading/scanner?limit=$limit');
    final list = (data as Map)['candidates'] as List;
    return list.map((e) => Candidate.fromJson((e as Map).cast<String, dynamic>())).toList();
  }

  Future<Map<String, dynamic>> autopilotRun(String accountId, {int topK = 10, int lookbackDays = 180}) async {
    final data = await _client.post('/trading/accounts/$accountId/autopilot/run?top_k=$topK&lookback_days=$lookbackDays');
    return (data as Map).cast<String, dynamic>();
  }

  Future<Map<String, dynamic>> report(String accountId) async {
    final data = await _client.get('/trading/accounts/$accountId/report');
    return ((data as Map)['summary'] as Map).cast<String, dynamic>();
  }
